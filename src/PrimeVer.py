...
# Retrive the FM version and Date from TI-file (*.txt)
...
currdir = os.getcwd()
if len(sys.argv) < 2:
    tempfile = filedialog.askopenfilename(parent=root, initialdir=currdir, title='Select a file',filetypes = (("txt files","*.txt"),("all files","*.*")))
else:
